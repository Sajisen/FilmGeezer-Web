  ...
  return (
    ...
    >
      <MediaDetailsContainer>
        <div className="min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-4 sm:p-6 lg:p-8">
          <header>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
              Watch the main trailer or choose another available video.
            </p>
          </header>

          <div className="mt-6 grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
            <div className="min-w-0">
              <div className="relative aspect-video min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-slate-950 shadow-2xl shadow-black/25">
                {isPlayerLoaded ? (
                  <iframe
                    src={embedUrl}
                  ...
                  >
                    ...
                    >
                      <div className="max-w-[88%] sm:max-w-[66%] lg:max-w-[52%]">
                        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-sky-300">
                          Video {selectedVideoIndex + 1} of {availableVideos.length}
                        </p>
              ...
              <div className="mt-4 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div aria-live="polite" className="min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-300">
                    {isPlayerLoaded ? "Now playing" : "Selected video"}
                  </p>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-400">
                    Choose a different trailer, teaser, or preview.
                  </p>
                </div>

                <span className="shrink-0 rounded-full border border-white/10 bg-white/[0.05] px-2.5 py-1 text-xs font-semibold text-slate-300">
                  {availableVideos.length}
                </span>
              </div>

              <div className="trailer-video-scroll mt-4 flex snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain pb-2 pr-2 lg:block lg:max-h-96 lg:space-y-2 lg:overflow-x-hidden lg:overflow-y-auto lg:pb-0 lg:pr-2">
                {availableVideos.map((video, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-[10.5rem] min-w-[10.5rem] snap-start rounded-2xl border p-3.5 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-sky-300 sm:w-48 sm:min-w-48 lg:w-full lg:min-w-0 lg:snap-none ${
                        isSelected
                          ? "border-sky-400/55 bg-sky-500/15 ring-1 ring-inset ring-sky-300/25"
