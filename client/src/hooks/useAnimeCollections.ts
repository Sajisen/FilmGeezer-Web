...
const initialRequestState: AnimeCollectionsRequestState = {
  requestKey: -1,
...
export function useAnimeCollections() {
  ...
  const [requestState, setRequestState] =
    useState<AnimeCollectionsRequestState>(initialRequestState);
  ...
  useEffect(() => {
    ...
    async function loadCollections() {
      try {
        const collections = await getAnimeCollections(controller.signal);
